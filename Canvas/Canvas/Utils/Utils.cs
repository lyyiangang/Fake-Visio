...
namespace Canvas
{
	...
	public class ScreenUtils
	{
        ...
        //convert unit rect to screen rect
        ...
        //convert screen pixel coordinate to unit
	...
	public class HitUtil
	{
        ...
        delegate  float OvalFun(UnitPoint center, float a, float b,UnitPoint pTest);
        public static bool IsPointOnOval(UnitPoint center,float halfWidth,float halfHeight,UnitPoint testPoint,float threshold )
        {
            OvalFun fun = (pCenter, a, b,pTest) => {
                float xx = (float)(pCenter.X - pTest.X);
                float yy = (float)(pCenter.Y - pTest.Y);
                return xx*xx/ (a * a) + yy*yy/ (b * b) - 1.0f;
            };
